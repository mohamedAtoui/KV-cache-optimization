"""
GPTNeo Decoder-Only Transformer Architecture

Based on "Attention is Not All You Need Anymore" and GPT architecture.
Implements a decoder-only transformer for causal language modeling on TinyStories.

Trained on TinyStories dataset as described in:
...
Architecture:
    ...
    - Causal self-attention throughout
...
from .attention import MultiHeadedAttention, create_causal_mask
from .layers import LayerNorm, PositionwiseFeedForward


class GPTNeoBlock(nn.Module):
    """
    Single GPTNeo Decoder Block
...
class GPTNeoModel(nn.Module):
    """
    GPTNeo Decoder-Only Transformer Model
...
class GPTNeoForCausalLM(nn.Module):
    """
    GPTNeo Model with Language Modeling Head
...
def create_gptneo_model(config):
    """
    Factory function to create GPTNeo model from config dict
    ...
    Returns:
        GPTNeoForCausalLM model instance

    Example:
        >>> config = {'vocab_size': 50257, 'hidden_size': 768, 'num_layers': 8}
...
if __name__ == "__main__":
    # Unit tests for GPTNeo architecture
    print("Testing GPTNeo Decoder-Only Architecture...")
    print("=" * 70)
    ...
    # Test 1: GPTNeoBlock
    print("1. Testing GPTNeoBlock...")
    block = GPTNeoBlock(hidden_size, num_heads, intermediate_size, dropout=0.1).to(device)
    ...
    print("   ✓ GPTNeoBlock working correctly")

    # Test 2: GPTNeoModel
    print("\n2. Testing GPTNeoModel...")
    ...
    print("   ✓ GPTNeoModel working correctly")

    # Test 3: GPTNeoForCausalLM
    print("\n3. Testing GPTNeoForCausalLM...")
    ...
    print("   ✓ GPTNeoForCausalLM working correctly")
    ...
    # Test 7: create_gptneo_model factory
    print("\n7. Testing create_gptneo_model() factory...")
    config = {
        'vocab_size': vocab_size,
        'hidden_size': 512,
        'num_layers': 4,
    ...
    print("\n8. Testing Weight Tying...")
    are_tied = model.lm_head.weight is model.transformer.token_embedding.weight
    print(f"   LM head and embeddings tied: {are_tied}")
    assert are_tied, "Weights should be tied"
    print("   ✓ Weight tying verified")

    print("\n" + "=" * 70)
    print("✓ All GPTNeo tests passed!")
    print("Decoder-only architecture ready for training on TinyStories")
